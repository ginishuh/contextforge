...
import {
  ...
  bootstrapResult,
  bootstrapResultSummary,
  bootstrapSessionWorkingContext,
  bootstrapSummary,
  bootstrapTrustForType,
  bootstrapUseHint,
  bootstrapWorkingSummary,
  compactAgentBootstrap,
  compactBootstrapCandidate,
  requiresLiveStateVerification,
  storageBootstrapInfo,
  workspaceBootstrapResult,
  workspaceResultSortScore,
} from './application/bootstrap_context.js';
import {
  clampImportance,
  contentHash,
  errorSummary,
...
import { distillMethods } from './distill/methods.js';
...
import { embeddingJobHelpers } from './embeddings/jobs.js';
import { embeddingMethods } from './embeddings/methods.js';
...
export function createContextForge(options = {}) {
  ...
  // enqueueEmbeddingSources from remember/promote/correct, and both handles are
  // injected into consolidationMethods and distillMethods the same way.
  const {
    enqueueEmbeddingSources,
    embeddingMaintenanceInventory,
    processEmbeddingJobBatch,
    embeddingFailureResult,
  } = embeddingJobHelpers({ config, embeddingProvider });
  ...
  return {
    ...
    ...consolidationMethods({
      config, useStore, memoryLifecycleForScope, getEffectiveRuntime,
      distillProviders, codexExec, runtimeFetchImpl, runDistillProvider,
      embeddingProvider, enqueueEmbeddingSources,
    }),
    ...embeddingMethods({
      config, embeddingProvider, useStore,
      embeddingMaintenanceInventory, enqueueEmbeddingSources, processEmbeddingJobBatch,
    }),
    ...distillMethods({
      config, useStore, operationKey, auditSourceKey, runDistillProvider,
      getEffectiveRuntime, getAutoPromoteAuditor,
      distillProviders, codexExec, runtimeFetchImpl,
      embeddingProvider, enqueueEmbeddingSources, embeddingFailureResult,
      providerConcurrencyLimit, CLOSEOUT_TRIGGERS, retryableAuditJobError,
      selectDistillWindow, sessionWorkingContextInput, rethrowExternalProviderTestError,
    }),
    ...
    routeAuditedMemoryCandidates(options = {}) {
      ...
      return useStore((store) => routeAuditedMemoryCandidates({
        store, scope, options, positiveNumber, truthyOption,
      }));
    },
        ...
        for (const item of selected) {
          ...
          try {
            ...
            const routed = persistApprovedAuditRouting({
              store, scope, auditedCandidate, audit, assertLease: assertJobLease, errorSummary,
            });
          ...
          proposals: [
            ...
            ...audited.map((item, index) =>
              auditedPromotionProposal(item.candidate, item.warnings, item.audit, storedAudited.length + index + 1, {
                ...
                promotionRouting: promotionRoutingResult(item.promotionRouting),
