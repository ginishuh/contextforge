...
cat >"$unit_path" <<EOF
[Unit]
Description=ContextForge agent ingest router for claude_code (${name})
...
echo "Installed claude_code agent router unit: ${unit_path}"
echo "Repo registry: ${repo_registry}"
echo "Enabled Claude Code repos: ${repo_count}"
