...
ExecStart=${env_bin} CONTEXTFORGE_STORAGE_MODE=remote CONTEXTFORGE_REMOTE_URL=${remote_url} ${node_bin} ${repo_root}/src/cli.js ingestCodexRoutedSessions --sessionsDir ${sessions_dir} --repoRegistry ${repo_registry} --sinceMinutes ${since_minutes} --distill ${distill} --watch --intervalMs ${interval_ms}
Restart=always
RestartSec=10
