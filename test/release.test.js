...
test('packaged agent guide links to standalone operational documentation', async () => {
  const guide = await fs.readFile('docs/agent-guide.md', 'utf8');
  const links = [...guide.matchAll(/\]\(([^)]+\.md)\)/g)].map((match) => match[1]);
  assert.ok(links.length > 0);
  for (const link of links) {
    await fs.access(path.resolve('docs', link));
  }
  assert.match(guide, /Distillation failure must not erase raw evidence/);
  assert.match(guide, /connection\.accessMode/);
  assert.match(guide, /does\s+not create a session/);
});
